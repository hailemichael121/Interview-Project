...
export async function apiFetch(url: string, options: RequestInit = {}) {
  ...
  const headers = {
    ...
    ...(session.data?.session
      ? {
          Authorization: `Bearer ${session.data.session.token}`, // Fixed template literal
        }
      : {}),
    ...options.headers,
  };

  const res = await fetch(`${apiBase}${url}`, { ...options, headers }); // Fixed template literal

  if (!res.ok) throw new Error(`API Error: ${res.status}`); // Fixed template literal
  return res.json();
}
